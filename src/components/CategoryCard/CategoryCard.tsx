...
function CategoryCard({ category, onClick, showControls, navigateTo }: CategoryCardProps) {
  ...
  return (
    <>
      ...
      <div className="w-full aspect-[2/1.5] group border border-purple-500 rounded-lg flex bg-center bg-no-repeat bg-cover relative hover:shadow-md overflow-hidden cursor-pointer" onClick={(event) => {
        event.stopPropagation();
        if (showControls) navigate(navigateTo)
        if (onClick) onClick()
      }}
        style={{
      ...
      >
        {showControls ? (<>
          <div className="group-hover:opacity-100 flex opacity-0 transition duration-200 ease-in-out absolute right-0 top-0 pr-2 pt-1 " >
            <Link to={routes.CATEGORY_SETTINGS(category.id)} onClick={(e) => e.stopPropagation()}>
            ...
            <PencilIcon className="h-6 w-6 text-purple-400 m-1 hover:text-purple-700 hover:scale-110 transition-transform duration-150 ease-in-out" onClick={(e) => {
              e.stopPropagation()
              setIsEditCategoryModalOpen(true)
            }} />
            <TrashIcon className="h-6 w-6 text-red-400 m-1 hover:text-red-600 hover:scale-110 transition-transform duration-150 ease-in-out" onClick={(e) => {
              e.stopPropagation()
